<script lang="ts">
    ...
    import Sources from "$lib/components/llmChat/Sources.svelte";
    ...
    const isLastMessageEmptyAssistant = $derived(
        lastMessage &&
    ...
    const shouldShowLoadingIndicator = $derived(
        isLoading && !isLastMessageEmptyAssistant,
    );

    // Show loading dots inside message if it's empty and we're loading
    const shouldShowLoadingInMessage = $derived(
        isLoading && isLastMessageEmptyAssistant,
    );
</script>
                        ...
                        <div class="flex-1">
                            ...
                            {:else if message.role === "assistant"}
                                <div class="text-xs text-gray-400 mt-1">
                                    No sources (sources={message.sources
                                        ? "exists"
                                        : "null"})
                                </div>
                            {/if}
